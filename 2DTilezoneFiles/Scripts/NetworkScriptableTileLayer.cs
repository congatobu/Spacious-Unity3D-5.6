...
[AddComponentMenu("2D/NetworkScriptableTileLayer")]
public class NetworkScriptableTileLayer : NetworkBehaviour, ITilezoneTileDataStream {
	...
	TileInfo CloneLayer( int newIndex, TileInfo layerToClone, int scriptableLayer, ITilezoneTileDataStream stream = null ) {
		...
		ti.transform.localScale = new Vector3( ti.zoomFactor, ti.zoomFactor, 1 );
...
//	void CreateLayers() {
//		int length = widthInChunks * heightInChunks;
//		for( int sl = 0; sl < scriptableLayers.Length; sl++ ) {
//			if( scriptableLayers[sl].layers == null )
//				scriptableLayers[sl].layers = new TileInfo[length];
//			if( scriptableLayers[sl].layers.Length != length ) {
//				TileInfo[] newWallLayers = new TileInfo[length];
//				for( int i = 0; i < scriptableLayers[sl].layers.Length && i < length; i++ ) {
//					newWallLayers[i] = scriptableLayers[sl].layers[i];
//				}
//				scriptableLayers[sl].layers = newWallLayers;
//			}
//			for( int i = 0; i < scriptableLayers[sl].layers.Length; i++ ) {
//				if( scriptableLayers[sl].layers[i] == null )
//					scriptableLayers[sl].layers[i] = CloneLayer( i, (i==0) ? scriptableLayers[sl].layers[0] : mainLayer, sl );
//			}
...
//	}
	...
	//		int length = widthInChunks * heightInChunks;
	//		if( wallLayers == null )
	//			_wallLayers = new TileInfo[length];
	//		if( wallLayers.Length != length ) {
	//			TileInfo[] newWallLayers = new TileInfo[length];
	//			for( int i = 0; i < wallLayers.Length && i < length; i++ ) {
	//				newWallLayers[i] = wallLayers[i];
	//			}
	//			_wallLayers = newWallLayers;
	//		}
	//		for( int i = 0; i < wallLayers.Length; i++ ) {
	//			if( wallLayers[i] == null )
	//				wallLayers[i] = CloneLayer( i, wallLayer );
	//		}
	...
	//			backgroundLayer = CloneLayer( -1, wallLayer  );
	//		}
	//
	//		if( backgroundLayers == null )
	//			_backgroundLayers = new TileInfo[length];
	//		if( backgroundLayers.Length != length ) {
	//			TileInfo[] newBackgroundLayers = new TileInfo[length];
	//			for( int i = 0; i < backgroundLayers.Length && i < length; i++ ) {
	//				newBackgroundLayers[i] = backgroundLayers[i];
	//			}
	//			_backgroundLayers = newBackgroundLayers;
	//		}
	//		for( int i = 0; i < backgroundLayers.Length; i++ ) {
	//			if( backgroundLayers[i] == null )
	//				backgroundLayers[i] = CloneLayer( i, wallLayers[i] );
	//		}
	//		ConnectLayers( wallLayers );
	//	}

//	public void InitAllLayers ( ) {
//
//		ClearLayers( );
//		CreateLayers( );
//
//	}
	...
	public virtual void Start () {
		if( isServer && this is ITilezoneContinousLoad )
			UpdateTilesContinuous();
	}


	void ChangeTileClient( int x, int y, int index, Tile tile, bool addToChangeList ) {
	...
	public void ChangeTile( int x, int y, int index, Tile tile, bool addToChangeList ) {
		if( UnityEditor.EditorApplication.isPlaying && !isServer ) {
			ChangeTileAndUpdate( x, y, index, tile, addToChangeList );
			return;
		}
		if( TrySetTile( x, y, index, tile ) )
		...
		if( addToChangeList )
			_changeDictionary[ index * width * height + y * width + x ] = tile.Serialize();
		if( !UnityEditor.EditorApplication.isPlaying ) {
			changesToSync[ index * width * height + y * width + x ] = tile.Serialize();
			addSyncToChangeList = addToChangeList;
		}
	}

	
	/// <summary>
	/// Changes the tile and updates the mesh and syncs the change over the network.
	/// </summary>
	/// <param name="x">The x coordinate.</param>
	/// <param name="y">The y coordinate.</param>
	/// <param name="index">Index.</param>
	...
	public void ChangeTileAndUpdate( int x, int y, int index, Tile tile, bool addToChangeList ) {
		if( addToChangeList )
			_changeDictionary[ index * width * height + y * width + x ] = tile.Serialize();
		if( UnityEditor.EditorApplication.isPlaying && !isServer ) {
			ChangeDataMessage msg = new ChangeDataMessage();
			msg.addToChangeList = addToChangeList;
			msg.changes = new Dictionary<int, int>();
			msg.changes.Add( index * width * height + y * width + x, tile.Serialize() );
			NetworkManager.singleton.client.Send( msgSendTileData, msg );
			return;
		}
		if( this as ITilezoneContinousLoad != null )
			( this as ITilezoneContinousLoad ).dataToLoad[ x, y, index ] = tile;
	...
	public virtual void OnValidate () {
		...
		for( int i = 0; i < scriptableLayers.Length; i++ ) {
			if( scriptableLayers[i].layers.Length == 0 )
	...
	public GridcastHit2D[] LinecastAll ( Vector2 start, Vector2 end, bool includeStartTile ) {
		...
		float tDeltaY = Mathf.Abs( 1 / v.y );
		if( v.x == 0 ) {
			tMaxX = Mathf.Infinity;
			tDeltaX = Mathf.Infinity;
		}
		if( v.y == 0 ) {
			tMaxY = Mathf.Infinity;
			tDeltaY = Mathf.Infinity;
		}
		int x = Mathf.FloorToInt( u.x );
		int y = Mathf.FloorToInt( u.y );
		if( tMaxX == 0 ) {
		...
		if( includeStartTile && IsCollision( x, y ) ) {
...
//		foreach( GridcastHit2D hit in result ) {
//			print( hit.xIndex + ": " + hit.yIndex );
//		}
	...
	//network code
	Dictionary<int, int> changesToSync = new Dictionary<int, int>();
	bool addSyncToChangeList;


	public const short msgSendInitialTileData = 2222;
	public const short msgSendTileData = 2223;
	public const short msgAskForInitialData = 2224;
	...
	public override bool OnSerialize (NetworkWriter writer, bool initialState) {
		...
		if( initialState )
			writer.Write( true );
		ChangeDataMessage msg = new ChangeDataMessage();
		msg.changes = changesToSync;
		msg.addToChangeList = addSyncToChangeList;
		writer.Write( msg );
		changesToSync.Clear();
		return true;
	}
	...
	public override void OnDeserialize (NetworkReader reader, bool initialState) {
		if( initialState && !reader.ReadBoolean() ) {
			NetworkManager.singleton.client.Send( msgAskForInitialData, new BlankMessage() );
			return;
		}
		ChangeDataMessage data = reader.ReadMessage<ChangeDataMessage>();
		foreach( KeyValuePair<int, int> change in data.changes ) {
			int x = change.Key % width;
			int index = change.Key / ( width * height );
			int y = change.Key / width - index * height;
			ChangeTileClient( x, y, index, Tile.DeSerialize( change.Value ), data.addToChangeList );
		}
		UpdateDirtyMeshes();
	}

	public class BlankMessage : MessageBase {
	...
	public class ChangeDataMessage : MessageBase {
		public Dictionary<int, int> changes;
		public bool addToChangeList;
		public override void Serialize (NetworkWriter writer) {
			writer.Write( addToChangeList );
		...
		public override void Deserialize (NetworkReader reader) {
			addToChangeList = reader.ReadBoolean();
			int count = (int)reader.ReadPackedUInt32();
			changes = new Dictionary<int, int>( count );
			for( int i = 0; i < count; i++ ) {
				changes.Add( reader.ReadInt32(), reader.ReadInt32() );
			}
		}
	}
	public class InitialChangeDataMessage : MessageBase {
		public enum DataMessageType {
			InitialMessage, FinalMessage, OnlyMessage, None
		}
		public Dictionary<int, int> changes;
		...
		public override void Serialize (NetworkWriter writer) {
			...
			if( type == DataMessageType.InitialMessage || type == DataMessageType.OnlyMessage )
				writer.Write( seed );
		...
		public override void Deserialize (NetworkReader reader) {
			...
			type = (DataMessageType)reader.ReadByte();
			if( type == DataMessageType.InitialMessage || type == DataMessageType.OnlyMessage )
				seed = reader.ReadInt32();
			int count = reader.ReadInt32();
			changes = new Dictionary<int, int>( count );
	...
	IEnumerator SendInitialDataCoroutine ( NetworkConnection conn ) {
		InitialChangeDataMessage msg = new InitialChangeDataMessage();
		msg.changes = new Dictionary<int, int>();
		msg.seed = seed;
		int i = 0;
		...
		foreach( KeyValuePair<int, int> change in _changeDictionary ) {
			...
			i++;
			if( i > 60 ) {
				msg.type = initialSend ? InitialChangeDataMessage.DataMessageType.InitialMessage : InitialChangeDataMessage.DataMessageType.None;
				initialSend = false;
				conn.Send( msgSendInitialTileData, msg );
				yield return null;
				msg.changes.Clear();
				i = 0;
				continue;
		...
		if( msg.changes.Count > 0 ) {
			msg.type = initialSend ? InitialChangeDataMessage.DataMessageType.OnlyMessage : InitialChangeDataMessage.DataMessageType.FinalMessage;
			conn.Send( msgSendInitialTileData, msg );
		}
		else if( initialSend ) {
			msg.type = InitialChangeDataMessage.DataMessageType.OnlyMessage;
			conn.Send( msgSendInitialTileData, msg );
		}
		yield return null;
	}
	void SendInitialData ( NetworkMessage msg ) {
		StartCoroutine( SendInitialDataCoroutine( msg.conn ) );
	}

	public virtual void OnRecievedInitialTileData () {
		if( this is ITilezoneContinousLoad )
			UpdateTilesContinuous();
	}

	void OnRecieveInitialData ( NetworkMessage msg ) {
		InitialChangeDataMessage data = msg.ReadMessage<InitialChangeDataMessage>();
		if( data.type == InitialChangeDataMessage.DataMessageType.InitialMessage || data.type == InitialChangeDataMessage.DataMessageType.OnlyMessage )
			seed = data.seed;
		foreach( KeyValuePair<int, int> change in data.changes ) {
			_changeDictionary[ change.Key ] = change.Value;
		}
		if( data.type == InitialChangeDataMessage.DataMessageType.FinalMessage || data.type == InitialChangeDataMessage.DataMessageType.OnlyMessage ) {
			OnRecievedInitialTileData();
		}
	}
	void OnRecieveChangeData ( NetworkMessage msg ) {
		ChangeDataMessage data = msg.ReadMessage<ChangeDataMessage>();
		foreach( KeyValuePair<int, int> change in data.changes ) {
			int x = change.Key % width;
			int index = change.Key / ( width * height );
			int y = change.Key / width - index * height;
			ChangeTile( x, y, index, Tile.DeSerialize( change.Value ), data.addToChangeList );
		}
		UpdateDirtyMeshes();
		SetDirtyBit( 1u );
	}
	public override void PreStartClient () {
		NetworkManager.singleton.client.RegisterHandler( msgSendInitialTileData, OnRecieveInitialData );
		if( isServer ) {
			NetworkServer.RegisterHandler( msgSendTileData, OnRecieveChangeData );
			NetworkServer.RegisterHandler( msgAskForInitialData, SendInitialData );
		}
	}
}
#endif