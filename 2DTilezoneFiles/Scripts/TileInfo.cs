	...
	public TilePrefabs tilePrefabs {
		get {
			if( _tilePrefabs == null )
				_tilePrefabs = GetComponent<TilePrefabs>();
			return _tilePrefabs;
		}
	}
...
//			}
	...
	public void UpdateVisualMesh ( bool fromEditor ) {
		if( !mapHasChanged )
			return;
		mapHasChanged = false;
		bool hasPrefabs = tilePrefabs != null;
		List<Collider2DEdge> edges = new List<Collider2DEdge>();
		List<Vector2[]> polygons = new List<Vector2[]>();

		#if UNITY_EDITOR
		if( hasPrefabs && UnityEditor.PrefabUtility.GetPrefabType( gameObject ) != UnityEditor.PrefabType.Prefab )
			tilePrefabs.RemoveAllPrefabs( fromEditor );
		#else
		if( hasPrefabs )
		tilePrefabs.RemoveAllPrefabs( fromEditor );
		#endif
...
//				UpdateAutoTile( x, y );
				...
				if( hasPrefabs && UnityEditor.PrefabUtility.GetPrefabType( gameObject ) != UnityEditor.PrefabType.Prefab ) {
					...
					TilePrefabs.TilePrefabInfo info = tilePrefabs.GetPrefabInfo( tiles[ y * mapWidth + x ] );
					if( info != null ) {
						tilePrefabs.AddPrefabObject( x, y, info );
						if( info.dontDrawTile )
							continue;
					}
				}
				#else
...
//		if( worldPos.y + 1 > ( transform.position.y + mapHeight ) * zoomFactor ) {
	...
	ITilezoneTileDataStream dataStream;
	Vector2 dataStreamPos;
	int dataStreamIndex;
	
	public void SetDataStream ( ITilezoneTileDataStream stream, Vector2 streamPos, int streamIndex ) {
		dataStream = stream;
		dataStreamPos = streamPos;
		dataStreamIndex = streamIndex;
	}
	public int GetWorldAutoTileIndex ( int localX, int localY ) {
		if( dataStream != null ) {
			return dataStream[ Mathf.FloorToInt( (transform.position.x - dataStreamPos.x) / zoomFactor ) + localX, Mathf.FloorToInt( (transform.position.y - dataStreamPos.y) / zoomFactor ) + localY, dataStreamIndex ].autoTileIndex;
		}
		if( localX < 0 ) {
			if( localY < 0 ) {
