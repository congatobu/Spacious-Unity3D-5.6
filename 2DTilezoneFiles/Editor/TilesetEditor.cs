...
public class TilesetEditor : EditorWindow {
	...
	static ScriptableTileLayerBase scriptableLayer;
	...
	void OnUndoRedo() {
		if (layerTileInfo != null) {
			...
			if( scriptableLayer == null && networkScriptableLayer == null ) {
				layerTileInfo.mapHasChanged = true;
				layerTileInfo.UpdateVisualMesh( true );
			}
			else if( networkScriptableLayer != null )
				networkScriptableLayer.UpdateDirtyMeshes();
			else
				scriptableLayer.UpdateMeshes();
			#else
			if( scriptableLayer == null ) {
				layerTileInfo.mapHasChanged = true;
				layerTileInfo.UpdateVisualMesh( true );
			}
			else
	...
	static void GetScriptableLayer () {
		scriptableLayer = layerToEdit.GetComponent<ScriptableTileLayerBase>();
		if( scriptableLayer == null && layerToEdit.transform.parent != null ) {
			ScriptableTileLayerBase possibleLayerBase = layerToEdit.transform.parent.GetComponent<ScriptableTileLayerBase>();
			if( possibleLayerBase != null ) {
				int index = possibleLayerBase.WorldPosToLayerIndex( layerToEdit.transform.position );
				for( int i = 0; i < possibleLayerBase.scriptableLayers.Length; i++ ) {
					if( possibleLayerBase.scriptableLayers[i].layers.Length > index && possibleLayerBase.scriptableLayers[i].layers[index] == layerTileInfo ) {
						scriptableLayer = possibleLayerBase;
						ScriptableTileLayerEditor.selectedLayer = i;
						return;
					}
				}
			}
	...
	#if UNITY_5_3 || UNITY_5_3_OR_NEWER
	static void GetNetworkScriptableLayer () {
		networkScriptableLayer = layerToEdit.GetComponent<NetworkScriptableTileLayer>();
		if( networkScriptableLayer == null && layerToEdit.transform.parent != null ) {
			NetworkScriptableTileLayer possibleLayerBase = layerToEdit.transform.parent.GetComponent<NetworkScriptableTileLayer>();
			if( possibleLayerBase != null ) {
				int index = possibleLayerBase.WorldPosToLayerIndex( layerToEdit.transform.position );
				for( int i = 0; i < possibleLayerBase.scriptableLayers.Length; i++ ) {
					if( possibleLayerBase.scriptableLayers[i].layers.Length > index && possibleLayerBase.scriptableLayers[i].layers[index] == layerTileInfo ) {
						networkScriptableLayer = possibleLayerBase;
						NetworkScriptableTileLayerEditor.selectedLayer = i;
						return;
					}
				}
			}
	...
	static NetworkScriptableTileLayer networkScriptableLayer;
	static void SetWireframeHidden ( bool isHidden ) {
		if( layerTileInfo == null )
			return;
		EditorUtility.SetSelectedWireframeHidden( layerTileInfo.meshRenderer, isHidden );
		if( scriptableLayer != null ) {
			for( int sl = 0; sl < scriptableLayer.scriptableLayers.Length; sl++ ) {
				for( int l = 0; l < scriptableLayer.scriptableLayers[sl].layers.Length; l++ ) {
					if( scriptableLayer.scriptableLayers[sl].layers[l] != null )
						EditorUtility.SetSelectedWireframeHidden( scriptableLayer.scriptableLayers[sl].layers[l].meshRenderer, isHidden );
				}
			}
		}
		else if( networkScriptableLayer != null ) {
			for( int sl = 0; sl < networkScriptableLayer.scriptableLayers.Length; sl++ ) {
				for( int l = 0; l < networkScriptableLayer.scriptableLayers[sl].layers.Length; l++ ) {
					if( networkScriptableLayer.scriptableLayers[sl].layers[l] != null )
						EditorUtility.SetSelectedWireframeHidden( networkScriptableLayer.scriptableLayers[sl].layers[l].meshRenderer, isHidden );
				}
			}
		}
	}
	...
	static void InitLayerToEdit () {
		EditorUtility.SetSelectedWireframeHidden(layerTileInfo.meshRenderer, true);
		meshRenderer = new SerializedObject( layerToEdit.GetComponent<MeshRenderer>() );
		meshRendererSortingLayer = meshRenderer.FindProperty( "m_SortingLayerID" );
		meshRendererSortingOrder = meshRenderer.FindProperty( "m_SortingOrder" );
		//		meshRenderer = layerToEdit.GetComponent<MeshRenderer>();
	...
	public static void CreateMesh( Rect pos, string goName, bool addRandomDungeon, bool addRandomPlatformer, bool addScriptableTileLayer, bool addNetworkScriptableTileLayer, bool addTilePrefabs ) {
		GameObject result;
		...
		if (addRandomPlatformer) {
			RandomPlatformer rp = result.AddComponent<RandomPlatformer>();
			rp.chunkWidth = (int)pos.width;
			rp.chunkHeight = (int)pos.height;
		}
		if( addScriptableTileLayer ) {
			ScriptableTileLayerBase sl = result.AddComponent<ScriptableTileLayerBase>();
			sl.chunkWidth = (int)pos.width;
			sl.chunkHeight = (int)pos.height;
		}
		#if UNITY_5_3 || UNITY_5_3_OR_NEWER
		if( addNetworkScriptableTileLayer ) {
			NetworkScriptableTileLayer sl = result.AddComponent<NetworkScriptableTileLayer>();
			sl.chunkWidth = (int)pos.width;
			sl.chunkHeight = (int)pos.height;
		}
		#endif
		if( addTilePrefabs ) {
			result.AddComponent<TilePrefabs>();
		}
	...
	void DrawSceneGrid() {
		...
		else if ( scriptableLayer != null ) {
			for (int x = 0; x <= scriptableLayer.width; x++) {
				Vector3 p1 = scriptableLayer.transform.position;
				p1.x += x*layerTileInfo.zoomFactor;
				
				Vector3 p2 = p1;
				p2.y += scriptableLayer.height*layerTileInfo.zoomFactor;
				
				Handles.DrawLine(p1, p2);
			}
			
			for (int y = 0; y <= scriptableLayer.height; y++) {
				Vector3 p1 = scriptableLayer.transform.position;
				p1.y += y*layerTileInfo.zoomFactor;
				
				Vector3 p2 = p1;
				p2.x += scriptableLayer.width*layerTileInfo.zoomFactor;
				
				Handles.DrawLine(p1, p2);
			}
		}
		else {
			for (int x = 0; x <= networkScriptableLayer.width; x++) {
				Vector3 p1 = networkScriptableLayer.transform.position;
				p1.x += x*layerTileInfo.zoomFactor;
				
				Vector3 p2 = p1;
				p2.y += networkScriptableLayer.height*layerTileInfo.zoomFactor;
				
				Handles.DrawLine(p1, p2);
			}
			
			for (int y = 0; y <= networkScriptableLayer.height; y++) {
				Vector3 p1 = networkScriptableLayer.transform.position;
				p1.y += y*layerTileInfo.zoomFactor;
				
				Vector3 p2 = p1;
				p2.x += networkScriptableLayer.width*layerTileInfo.zoomFactor;
	...
	void AddTile(Vector2 pos, Tile selectedTile) {
		if( scriptableLayer == null && networkScriptableLayer == null ) {
			...
			if( !scriptableLayer.IsInBounds( pos ) || !scriptableLayer.isLayersSetUp )
				return;
			int i = scriptableLayer.WorldPosToLayerIndex( pos );
			if ((scriptableLayer[ pos, ScriptableTileLayerEditor.selectedLayer ] == selectedTile && autoTileSelected < 0) || ( scriptableLayer[ pos, ScriptableTileLayerEditor.selectedLayer ].autoTileIndex == autoTileSelected && autoTileSelected >= 0 ) )
				return;
			if( scriptableLayer.scriptableLayers[0].layers[i] == null )
				scriptableLayer[ pos, ScriptableTileLayerEditor.selectedLayer ] = Tile.empty;
			if( scriptableLayer.scriptableLayers[ScriptableTileLayerEditor.selectedLayer].layers[i] != null )
				Undo.RecordObject(scriptableLayer.scriptableLayers[ScriptableTileLayerEditor.selectedLayer].layers[i], "Add Tile");
			Tile newTile = new Tile( selectedTile );
			if( autoTileSelected != -1 )
				newTile.autoTileIndex = autoTileSelected;
			scriptableLayer.ChangeTile( pos, ScriptableTileLayerEditor.selectedLayer, newTile, false );
		}
		else {
			if( !networkScriptableLayer.IsInBounds( pos ) || !networkScriptableLayer.isLayersSetUp )
				return;
			int i = networkScriptableLayer.WorldPosToLayerIndex( pos );
			if ((networkScriptableLayer[ pos, NetworkScriptableTileLayerEditor.selectedLayer ] == selectedTile && autoTileSelected < 0) || ( networkScriptableLayer[ pos, NetworkScriptableTileLayerEditor.selectedLayer ].autoTileIndex == autoTileSelected && autoTileSelected >= 0 ) )
				return;
			if( networkScriptableLayer.scriptableLayers[NetworkScriptableTileLayerEditor.selectedLayer].layers[i] == null )
				networkScriptableLayer[ pos, NetworkScriptableTileLayerEditor.selectedLayer ] = Tile.empty;
			if( networkScriptableLayer.scriptableLayers[NetworkScriptableTileLayerEditor.selectedLayer].layers[i] != null )
			Undo.RecordObject(networkScriptableLayer.scriptableLayers[NetworkScriptableTileLayerEditor.selectedLayer].layers[i], "Add Tile");
			Tile newTile = new Tile( selectedTile );
			if( autoTileSelected != -1 )
				newTile.autoTileIndex = autoTileSelected;
			networkScriptableLayer.ChangeTile( pos, NetworkScriptableTileLayerEditor.selectedLayer, newTile, false );
		}
	}
	...
	void FloodFill(Vector2 pos) {
		...
		if( scriptableLayer == null && networkScriptableLayer == null ) {
			...
			if( !scriptableLayer.IsInBounds( pos ) )
				return;
			TileInfo l = scriptableLayer.scriptableLayers[ ScriptableTileLayerEditor.selectedLayer ].layers[ scriptableLayer.WorldPosToLayerIndex( pos ) ];
			if( l == null ) {
				scriptableLayer[ pos, ScriptableTileLayerEditor.selectedLayer ] = Tile.empty;
				l = scriptableLayer.scriptableLayers[ ScriptableTileLayerEditor.selectedLayer ].layers[ scriptableLayer.WorldPosToLayerIndex( pos ) ];
			}
			Undo.RecordObject(l, "Flood Fill");
			if (autoTileSelected == -1)
				l.FloodFill(pos, selectedTiles, pos, new Tile(l.tiles[l.WorldPointToMapIndex(pos)]));
			else
				l.FloodFill(pos, autoTileSelected, pos, new Tile(l.tiles[l.WorldPointToMapIndex(pos)]));
			scriptableLayer.SetDirty( pos, ScriptableTileLayerEditor.selectedLayer );
		}
		else {
			if( !networkScriptableLayer.IsInBounds( pos ) )
				return;
			TileInfo l = networkScriptableLayer.scriptableLayers[ NetworkScriptableTileLayerEditor.selectedLayer ].layers[ networkScriptableLayer.WorldPosToLayerIndex( pos ) ];
			if( l == null ) {
				networkScriptableLayer[ pos, NetworkScriptableTileLayerEditor.selectedLayer ] = Tile.empty;
				l = networkScriptableLayer.scriptableLayers[ NetworkScriptableTileLayerEditor.selectedLayer ].layers[ networkScriptableLayer.WorldPosToLayerIndex( pos ) ];
			}
			Undo.RecordObject(l, "Flood Fill");
			if (autoTileSelected == -1)
				l.FloodFill(pos, selectedTiles, pos, new Tile(l.tiles[l.WorldPointToMapIndex(pos)]));
				...
				l.FloodFill(pos, autoTileSelected, pos, new Tile(l.tiles[l.WorldPointToMapIndex(pos)]));
			networkScriptableLayer.SetDirty( pos, NetworkScriptableTileLayerEditor.selectedLayer );
		}
	}
	...
	void RemoveTile(Vector2 pos) {
		if( scriptableLayer == null && networkScriptableLayer == null ) {
			...
			if( !scriptableLayer.IsInBounds( pos ) || !scriptableLayer.isLayersSetUp || scriptableLayer[ pos, ScriptableTileLayerEditor.selectedLayer ] == Tile.empty )
				return;
			Undo.RecordObject(scriptableLayer.scriptableLayers[ScriptableTileLayerEditor.selectedLayer].layers[scriptableLayer.WorldPosToLayerIndex( pos )], "Remove Tile");
			scriptableLayer.ChangeTile( pos, ScriptableTileLayerEditor.selectedLayer, Tile.empty, false );
		}
		else {
			if( !networkScriptableLayer.IsInBounds( pos ) || !networkScriptableLayer.isLayersSetUp || networkScriptableLayer[ pos, NetworkScriptableTileLayerEditor.selectedLayer ] == Tile.empty )
				return;

			Undo.RecordObject(networkScriptableLayer.scriptableLayers[NetworkScriptableTileLayerEditor.selectedLayer].layers[networkScriptableLayer.WorldPosToLayerIndex( pos )], "Remove Tile");
			networkScriptableLayer.ChangeTile( pos, NetworkScriptableTileLayerEditor.selectedLayer, Tile.empty, false );
		}
	}
	...
	void RotateTile(Vector2 pos) {
		if( scriptableLayer == null && networkScriptableLayer == null ) {
			...
			if( !scriptableLayer.IsInBounds( pos ) || !scriptableLayer.isLayersSetUp || scriptableLayer[ pos, ScriptableTileLayerEditor.selectedLayer ] == Tile.empty )
				return;
			Undo.RecordObject(scriptableLayer.scriptableLayers[ScriptableTileLayerEditor.selectedLayer].layers[scriptableLayer.WorldPosToLayerIndex( pos )], "Rotate Tile");
			Tile newTile = new Tile( scriptableLayer[ pos, ScriptableTileLayerEditor.selectedLayer ] );
			newTile.rotation--;
			if( newTile.rotation < 0 )
				newTile.rotation += 4;
			scriptableLayer.ChangeTile( pos, ScriptableTileLayerEditor.selectedLayer, newTile, false );
		}
		else {
			if( !networkScriptableLayer.IsInBounds( pos ) || !networkScriptableLayer.isLayersSetUp || networkScriptableLayer[ pos, 0 ] == Tile.empty )
				return;
			Undo.RecordObject(networkScriptableLayer.scriptableLayers[NetworkScriptableTileLayerEditor.selectedLayer].layers[networkScriptableLayer.WorldPosToLayerIndex( pos )], "Rotate Tile");
			Tile newTile = new Tile( networkScriptableLayer[ pos, NetworkScriptableTileLayerEditor.selectedLayer ] );
			newTile.rotation--;
			if( newTile.rotation < 0 )
				newTile.rotation += 4;
			networkScriptableLayer.ChangeTile( pos, NetworkScriptableTileLayerEditor.selectedLayer, newTile, false );
	...
	void FlipTile(Vector2 pos) {
		if( scriptableLayer == null && networkScriptableLayer == null ) {
			...
			if( !scriptableLayer.IsInBounds( pos ) || !scriptableLayer.isLayersSetUp || scriptableLayer[ pos, ScriptableTileLayerEditor.selectedLayer ] == Tile.empty )
				return;
			Undo.RecordObject(scriptableLayer.scriptableLayers[ScriptableTileLayerEditor.selectedLayer].layers[scriptableLayer.WorldPosToLayerIndex( pos )], "Flip Tile");
			Tile newTile = new Tile( scriptableLayer[ pos, ScriptableTileLayerEditor.selectedLayer ] );
			newTile.flip = !newTile.flip;
			scriptableLayer.ChangeTile( pos, ScriptableTileLayerEditor.selectedLayer, newTile, false );
		}
		else {
			if( !networkScriptableLayer.IsInBounds( pos ) || !networkScriptableLayer.isLayersSetUp || networkScriptableLayer[ pos, NetworkScriptableTileLayerEditor.selectedLayer ] == Tile.empty )
				return;
			Undo.RecordObject(networkScriptableLayer.scriptableLayers[NetworkScriptableTileLayerEditor.selectedLayer].layers[networkScriptableLayer.WorldPosToLayerIndex( pos )], "Flip Tile");
			Tile newTile = new Tile( networkScriptableLayer[ pos, NetworkScriptableTileLayerEditor.selectedLayer ] );
			newTile.flip = !newTile.flip;
			networkScriptableLayer.ChangeTile( pos, NetworkScriptableTileLayerEditor.selectedLayer, newTile, false );
		}
	...
	public void OnSceneGUI(SceneView sceneView) {
		...
		if( scriptableLayer != null ) {
			ScriptableTileLayerEditor.SceneGUI( scriptableLayer );
		}
		#if UNITY_5_3 || UNITY_5_3_OR_NEWER
		if( networkScriptableLayer != null ) {
			NetworkScriptableTileLayerEditor.SceneGUI( networkScriptableLayer );
		}
		#endif
		...
		if (mouseDownScene && toolSelected == TileTool.Draw && !current.shift ) {
			...
			if( scriptableLayer == null && networkScriptableLayer == null )
				...
				scriptableLayer.UpdateDirtyMeshes();
			#else
			if( scriptableLayer == null )
				layerTileInfo.UpdateVisualMesh( true );
			else
	...
	void BakeAutoTile( int index ) {
		...
		if( EditorUtility.DisplayDialog( "Bake Autotile", "This will remove all " + layerTileInfo.autoTileNames[index] + " Autotiles from this layer and replace them with the same tile without being an Autotile.", "OK", "Cancel" ) ) {
			...
			if( scriptableLayer == null && networkScriptableLayer == null ) {
				...
				foreach( TileInfo ti in scriptableLayer.scriptableLayers[ScriptableTileLayerEditor.selectedLayer].layers ) {
					Undo.RecordObject( ti, "Bake Autotile" );
					ti.BakeAutoTile( index );
				}
			}
			else {
				foreach( TileInfo ti in networkScriptableLayer.scriptableLayers[NetworkScriptableTileLayerEditor.selectedLayer].layers ) {
					Undo.RecordObject( ti, "Bake Autotile" );
					ti.BakeAutoTile( index );
				}
			}
...
//			}
			...
			if (layerTileInfo != null) {
				...
				EditorGUILayout.Space();
				if( GUILayout.Button( "Save Data Asset" ) ) {
					GUILayout.EndScrollView();
				...
				GUILayout.BeginHorizontal();
				GUILayout.Label( "Layer Hash: " );
				EditorGUILayout.SelectableLabel( layerTileInfo.GetHashCode().ToString(), EditorStyles.helpBox, GUILayout.Height( EditorGUIUtility.singleLineHeight ) );
				GUILayout.EndHorizontal();
				EditorGUILayout.Space();
	...
	void SaveDataAsset () {
		TileDataAsset newData = ScriptableObject.CreateInstance<TileDataAsset>();
		newData._width = layerTileInfo.mapWidth;
		newData._height = layerTileInfo.mapHeight;
		newData.tiles = (Tile[])layerTileInfo.tiles.Clone();
		newData.layerHashes = new int[1] { layerTileInfo.GetHashCode() };
		newData._numberOfLayers = 1;
		
		AssetDatabase.CreateAsset( newData, "Assets/" + layerToEdit.name + "Data.asset" );
...
public class CreateTileLayer : EditorWindow {
	...
	bool createRandomPlatformer;
	bool createScriptableTileLayer;
	bool createNetworkScriptableTileLayer;
	bool createTilePrefabs;
	...
	public void OnGUI() {
		...
		GUILayout.Space( 16 );
		EditorGUILayout.HelpBox( "The following components can also be added manually after the layer is created", MessageType.Info );
		createTilePrefabs = EditorGUILayout.Toggle( "Create Tile Prefabs", createTilePrefabs );
		EditorGUI.BeginChangeCheck();
		createRandomDungeon = EditorGUILayout.Toggle("Create Random Dungeon", createRandomDungeon);
		if( EditorGUI.EndChangeCheck() ) {
			if( createRandomDungeon ) {
				createRandomPlatformer = false;
				createScriptableTileLayer = false;
				createNetworkScriptableTileLayer = false;
			}
		}
		EditorGUI.BeginChangeCheck();
		createRandomPlatformer = EditorGUILayout.Toggle("Create Random Platformer", createRandomPlatformer);
		if( EditorGUI.EndChangeCheck() ) {
			if( createRandomPlatformer ) {
				createRandomDungeon = false;
				createScriptableTileLayer = false;
				createNetworkScriptableTileLayer = false;
			}
		}
		EditorGUI.BeginChangeCheck();
		createScriptableTileLayer = EditorGUILayout.Toggle("Create Scriptable Tile Layer", createScriptableTileLayer);
		if( EditorGUI.EndChangeCheck() ) {
			if( createScriptableTileLayer ) {
				createRandomDungeon = false;
				createRandomPlatformer = false;
				createNetworkScriptableTileLayer = false;
			}
		...
		#if UNITY_5_3 || UNITY_5_3_OR_NEWER
		EditorGUI.BeginChangeCheck();
		createNetworkScriptableTileLayer = EditorGUILayout.Toggle("Create Network Scriptable Tile Layer", createNetworkScriptableTileLayer);
		if( EditorGUI.EndChangeCheck() ) {
			if( createNetworkScriptableTileLayer ) {
				createRandomDungeon = false;
				createRandomPlatformer = false;
				createScriptableTileLayer = false;
			}
		...
		if (GUILayout.Button("Create Layer")) {
			...
			TilesetEditor.CreateMesh(mapRect, goName, createRandomDungeon, createRandomPlatformer, createScriptableTileLayer, createNetworkScriptableTileLayer, createTilePrefabs);
